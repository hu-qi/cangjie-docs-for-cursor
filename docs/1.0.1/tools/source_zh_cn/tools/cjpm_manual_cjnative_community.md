...
### 已知问题
  ...
  1. 若源码目录结构存在空格，或者 `cjpm.toml` 中配置项配置的路径存在空格，`test` 和 `bench` 命令可能会出现错误。为规避该类问题，请更改目录结构，避免带有空格的目录存在。

  2. `Windows` 系统中，若源码目录结构存在 `.`，或者 `cjpm.toml` 中配置项配置的路径存在 `.`，`cjpm` 部分命令可能会出现错误。为规避该类问题，请更改目录结构，避免带有 `.` 的目录存在。

### 交叉编译使用说明

`CJPM` `cjnative` 后端支持将项目交叉编译到其他平台并运行。例如，假设目标平台为 `arch-sys-abi`，编译步骤如下：
